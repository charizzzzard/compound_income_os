...
from src.personal_kpi_provenance_audit import run_personal_kpi_provenance_audit
...
class PersonalKpiProvenanceAuditTests(unittest.TestCase):
    def setUp(self) -> None:
        self.tmp = ROOT / "tests" / "_tmp_kpi_provenance"
        if self.tmp.exists():
        ...
        self.audit_output = self.tmp / "audit.csv"
    ...
    def test_value_present_without_source_reference_is_partial(self) -> None:
        self.write_base_inputs(
            registry_rows=[
                {"ticker": "AAA", "isin": "US1", "company_name": "Alpha", "company_type_profile": "STANDARD", "kpi_name": "revenue_cagr_5y", "source_type": "SEC", "source_name": "CompanyFacts", "source_reference": "", "source_as_of_date": "2026-01-02", "verification_status": "REVIEWED", "data_quality_flag": "OK", "evidence_present": "True", "reported_value": "12.3"}
            ]
        )
        self.run_audit()
    ...
    def test_score_audit_value_without_evidence_or_applied_match_is_partial(self) -> None:
        ...
        row = read_csv(self.audit_output)[0]
        self.assertEqual(row["source_layer"], "SCORE_AUDIT")
        self.assertEqual(row["provenance_status"], "PARTIAL")
        self.assertEqual(row["reason_code"], "EVIDENCE_REGISTRY_MISSING")

    def test_multiple_registry_candidates_are_ambiguous(self) -> None:
        self.write_base_inputs(
            registry_rows=[
    ...
    def test_missing_inputs_create_deterministic_empty_outputs(self) -> None:
        ...
        self.assertEqual(read_csv(self.audit_output), [])
        self.assertEqual(self.summary_value("audit_rows_total"), "0")
        self.assertNotEqual(self.summary_value("warnings_total"), "0")

    def test_private_paths_are_sanitized_in_report(self) -> None:
        self.write_base_inputs()
        self.run_audit(raw_master_input="data/raw/private/fundamentals/private_master.csv")
