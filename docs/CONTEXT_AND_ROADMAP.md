...
- `src.personal_run_engine` fuer explizite persoenliche Stage-Orchestrierung, Run-Manifest, Artefaktindex, Run-Report und die read-only `decision_quality`-, `decision_journal_validation`- und `dashboard_operator_summary`-Stages
...
- `src.dashboard_operator_summary` fuer eine read-only maschinenlesbare Operator-Summary aus Decision Quality, Decision Journal Validation und Review Queue ohne visuelles Dashboard und ohne neue Investmentlogik
- `src.data_freshness` fuer eine standalone read-only Freshness-/Staleness-Summary aus expliziten Datumsfeldern und `configs/data_freshness_thresholds.yaml`; keine `personal_run_engine`-Stage in diesem Patch
...
20. Dashboard Operator Summary: `src.dashboard_operator_summary` erzeugt `data/processed/review_queue_summary.json` aus Decision Quality, Decision Journal Validation und Review Queue. Die Stage `dashboard_operator_summary` laeuft nach `decision_journal_validation`, bleibt read-only, erzeugt kein visuelles Dashboard und fuehrt keine neue Investmentlogik ein. Missing/Partial-Artefakte bleiben explizit sichtbar; Header-only Validation/Queue-Artefakte ergeben `PASS` mit Nullzaehlern.
21. Data Freshness / Staleness: `docs/contracts/DATA_FRESHNESS_STALENESS_CONTRACT.md` definiert das kanonische Statusmodell fuer `FRESH`, `STALE`, `MISSING`, `UNKNOWN`, `REVIEW_REQUIRED` und `NOT_APPLICABLE`. `src.data_freshness` kann aus expliziten Datumsfeldern und `configs/data_freshness_thresholds.yaml` eine standalone Summary erzeugen; fehlende oder unbekannte Freshness wird nicht als `FRESH` interpretiert. Eine `personal_run_engine`-Stage ist bewusst Folgearbeit.
