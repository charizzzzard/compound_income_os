...
- `src.performance_engine`, `src.portfolio_history_engine`, `src.cost_tax_engine` und `src.dashboard_engine` fuer Performance-/Benchmark-, Portfolio-Historien-, Cost-/Tax- und Dashboard-Artefakte
...
7. Performance und Benchmark: `src.portfolio_history_engine` baut aus expliziten Positions-Snapshots Archiv-/Timeseries-Artefakte; `src.performance_engine` erzeugt Benchmark-/Performance-Artefakte mit Snapshot-, Perioden- und expliziten Historien-KPIs.
8. Cost/Tax: `src.cost_tax_engine` erzeugt Ledger-, Summary-, KPI- und Report-Artefakte anhand belegter manueller oder Dokumentdaten.
