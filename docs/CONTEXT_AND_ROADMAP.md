...
18. Decision Quality State: `src.personal_decision_quality_state` erzeugt aus bestehenden Input-Closure-, Decision-Capture-, Run-Manifest-/Used-Inputs-, Cash-Refill-, Rebalance-, Ranking- und Score-Audit-Artefakten `data/processed/decision_quality_state.csv`, `data/processed/decision_quality_state.json` und `reports/YYYY-MM-DD/decision_quality_report.md`. Der Schritt ist ein deterministischer Review-/Governance-Producer und ist read-only als `decision_quality`-Stage in `src.personal_run_engine` integriert; die Run Engine schreibt vor dieser Stage eine vorlaeufige aktuelle Manifest-/Used-Inputs-Lineage und finalisiert danach Manifest und Artifact Index erneut. Ranking Robustness, Sensitivity, Scenario und Tail Risk bleiben in Phase 1.5B `NOT_EVALUATED`. Es gibt keine Broker-, Order-, Scoring-, Portfolio-Regel-, Simulation-, Backtesting- oder Outcome-Attribution-Logik.
