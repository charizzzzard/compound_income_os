...
11. Phase 1 Step 1 Sparplan: src.savings_plan_registry validates a manual savings plan registry and produces summary/report artifacts without routing logic.
12. KPI Tier Guardrails: Fundamentals-, Scoring- und Monthly-Ranking-Pfade unterscheiden Core-Quality-, Valuation-, Dividend-FCF- und Advanced-Datenqualitaet. Fehlende Tier-Daten bleiben sichtbar und werden konservativ in Score-/Monthly-Guardrails verarbeitet.
13. SEC-Archiv: 26 ehemalige SEC-Module und 26 zugehoerige Tests liegen als read-only Referenz unter `_archive/sec/`. Sie sind nicht Teil der aktiven `src/`-Oberflaeche und werden durch `python -m unittest discover -s tests -p "test_*.py"` nicht mehr entdeckt.
14. Personal-Meta-Archiv: `src.personal_profile_review_materialize` und der zugehoerige Test liegen seit Patch 2.2b als read-only Referenz unter `_archive/personal_meta/`. Sie sind nicht Teil der aktiven `src/`-Oberflaeche und werden durch `python -m unittest discover -s tests -p "test_*.py"` nicht mehr entdeckt.
