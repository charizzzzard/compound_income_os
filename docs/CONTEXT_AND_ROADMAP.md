...
- `src.scoring_engine`, `src.fundamentals_engine`, `src.fundamentals_master`, `src.external_sec_companyfacts_fetch`, `src.fundamentals_evidence_engine`, `src.fundamentals_overlay_engine`, `src.valuation_engine` und `src.company_master` fuer Score-, Fundamentals-, expliziten SEC-Snapshot-Fetch, Personal-Master-, Evidence-/Research-Backlog-, Overlay-/Applied-Master- und Bewertungslogik
...
3. Personal-Master/Evidence/Overlay: `src.fundamentals_master` validiert lokale Personal-Fundamentals, matched Holdings konservativ und erzeugt Coverage-/Research-Gap-Artefakte; `src.external_sec_companyfacts_fetch` kann explizit und read-only SEC CompanyFacts in ein lokales Snapshot-Artefakt fuer den bestehenden Snapshot-Pfad schreiben; `src.fundamentals_evidence_engine` validiert explizite lokale KPI-Evidence und erzeugt Registry-/Research-Backlog-Artefakte ohne Rueckschreibung in den Master; `src.fundamentals_overlay_engine` validiert explizite Analyst-Overlays und erzeugt eine Applied-Master-Projektion ohne Mutation des Original-Masters.
